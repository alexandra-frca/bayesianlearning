...
lbound = np.array([0,0,0]) # The left boundaries for the parameters.
rbound = np.array([1,1,1]) # The right boundaries for the parameters.
...
def measure(t):
    ...
    p = np.random.binomial(1, p=np.sum(np.cos(real_parameters*t/2)**2/3))
    if (r<p):
        return 1
    return 0
...
def simulate_1(particle, t):
    ...
    '''
    p=np.sum(np.cos(particle*t/2)**2/3)
    return p 
...
def U_gradient(data,particle,autograd=False):
    ...
    if autograd:
        ...
        for (t,outcome) in data:
            f = np.sum(np.cos(particle*t/2)**2)/3
            if outcome==1:
                DU+=t/3*np.sin(particle*t/2)*np.cos(particle*t/2)/f
                '''
                DU+=np.array([t/2*np.sin(particle[0]*t/2)*
                              np.cos(particle[0]*t/2)/f,
                              t/2*np.sin(particle[1]*t/2)*
                              np.cos(particle[1]*t/2)/f])'''
            if outcome==0:
                DU+=-t/3*np.sin(particle*t/2)*np.cos(particle*t/2)/(1-f)
                '''
                DU+=np.array([-t/2*np.sin(particle[0]*t/2)*
                              np.cos(particle[0]*t/2)/(1-f),
                              -t/2*np.sin(particle[1]*t/2)*
                              np.cos(particle[1]*t/2)/(1-f)])'''
    return(DU)

test_differentiation = False
if test_differentiation:
    data = np.array([(1.7,0),(2.4,0)])
    particle = np.array([0.3,0.9,0.7])
...
def metropolis_hastings_step(data, particle, S=np.identity(3),
                             factor=0.1,
    ...
    if first_metropolis_hastings_step:
      if not np.array_equal(S,np.identity(3)):
          Cov = "Cov"
      else:
          Cov = "I"
...
def hamiltonian_MC_step(data, particle, 
                        M=np.identity(3), L=10, eta=0.005, threshold=0.1):
    '''
    ...
    if (threshold<1):
        ...
        initial_momentum = np.random.multivariate_normal([0,0,0], M)
        new_particle, p = \
...
def offline_estimation(distribution, data, threshold=N_particles/2):
    '''
    ...
    if first_offline_estimation is True:
        print("Offline estimation: random times <= 100")
...
def plot_distribution(distribution, real_parameters):
...
def main():
    global real_parameters, N_particles
    ...
    steps = 200
    ts = [100*random.random() for k in range(steps)] 
