...
def measure(t):
    ...
    p = np.random.binomial(1, p=np.sum(np.cos(real_parameters*t/2)**2/2))
    if (r<p):
        return 1
    return 0
...
def simulate_1(particle, t):
    ...
    '''
    p=np.sum(np.cos(particle*t/2)**2/2)
    return p 
...
def U_gradient(data,particle,autograd=False):
    ...
    if autograd:
        ...
        for (t,outcome) in data:
            f = np.sum(np.cos(particle*t/2)**2)/2
            if outcome==1:
...
def hamiltonian_MC_step(data, particle, 
                        M=np.identity(2), L=10, eta=0.005, threshold=0.1):
    '''
