...
Includes 2 functions for offline estimation; one receives a complete set of data 
and performs inference based on it, and the other one picks the evolution times 
according to some input parameters so as to tendentially increase them as the 
inference process advances. 
...
first_offline_estimation_0 = True
def offline_estimation_0(distribution, data, threshold=None, chunksize=1,
                       plot_all=False):
    '''
    ...
    '''
    global first_offline_estimation_0
    if first_offline_estimation_0 is True:
        print("Estimation: data chunksize = ", chunksize)
        first_offline_estimation_0 = False
...
def main():
    ...
    if test_resampling:
        ...
        for i in range(groups):
            ...
            dist, data = offline_estimation(prior.copy(),measurements,
