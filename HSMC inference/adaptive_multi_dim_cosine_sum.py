...
def adaptive_estimation(distribution, updates, threshold=None, 
    ...
    for i in range(updates):
        ...
        occrate = space_occupation(particles)
...
def dict_to_list(distribution):
    '''
    Converts a distribution dictionary representation to a list representation,
    a sequence of pairs consisting of parameter vectors and particle weights.
    ...
    distribution: dict
        , with (key,value):=(particle,importance weight) 
        , and particle the parameter vector (as a bit string)
        The distribution (SMC approximation).
        
    Returns
    -------
    particles: [([float],float)]
        The corresponding list of (parameter vector,importance weight) tuples. 
    '''
    particles = []
    for key in distribution:
        particle = np.frombuffer(key,dtype='float64')
