...
def measure(t):
    ...
    p = np.random.binomial(1, p=np.sum(np.cos(real_parameters*t/2)**2/4))
    if (r<p):
        return 1
    return 0
...
if test_differentiation:
    ...
    particle = np.array([0.3,0.9,0.7,0.25])
...
def offline_estimation(distribution, data, threshold=None):
    ...
    threshold: float, optional
        The threshold effective sample size that should trigger a resampling 
        step (Default is None, N_particles/2 will be used). 
...
def plot_distribution(distribution, real_parameters, n_graphs=1, note=""):
...
def print_HMC_stats():
    global total_HMC, accepted_HMC, total_MH, accepted_MH,N_particles
...
def main():
    global real_parameters, N_particles, dim
    ...
    if test_no_resampling: # Just for reference.
        ...
        plot_distribution(dist_no_resampling,real_parameters,n_graphs=2,
                          note="(no resampling)")
    ...
    if test_resampling:
        ...
        N_particles = 10**dim
        prior = uniform_prior()
        final_dist = offline_estimation(prior.copy(),data,threshold=500)
        plot_distribution(final_dist,real_parameters,n_graphs=2)

    print("\nn=%d; N=%d; %dd" % (N_particles,steps,dim))
    print_HMC_stats()
