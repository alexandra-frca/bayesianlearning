...
offline and adaptive Bayesian inference and using a single weak simulator call 
per step (i.e. with no reconstruction of the likelihood function, just a sample
taken from it at each iteration).
...
def simulate(test_f, t, runs=1):
    '''
    Provides points sampled from the likelihood  P(D=1|test_f,t) of an x-spin
    ...
    runs: int, optional
        ...
        which will determine the accuracy of the estimate (Default is 1 for the 
        likelihood-free approach - the result is then binary).
        
    Returns
    -------
    p: float
        The fraction of times the particle was found at state |+> (for runs=1,
...
def bayes_update(distribution, t, outcome, threshold=N_particles/2):
    ...
    '''
    global N_particles
    for particle in list(distribution):
        s = simulate(float(particle),t)
        if (outcome!=s):
            del distribution[particle] # Equivalently, set the weight to zero.
            #print("died")
            #print(particle)
    
    for particle in distribution:   
        # The remaining particles get uniform weights.
        distribution[particle]=1/len(distribution)
        
    # Sample the unused particles according to the distribution arising from  
    #the surviving ones.
    new_distribution = distribution.copy()
    for i in range(N_particles-len(distribution)):
...
def offline_estimation(distribution, f_max, steps, increment=0.08):
    ...
    '''
    # We'll use evenly spaced times, fixed in advance (offline).
