...
def plot_curve(xs,ys, note=""):
    ...
    axs.set_title("Kernel density estimate " + note)
...
def plot_kde(distribution,note=""):
    ...
    xs = np.linspace(glob.lbound[0],glob.rbound[0],1000)
    ys = [weighted_kernel_density_estimate(x,ks,stdev) for x in xs]
    plot_curve(xs,ys,note)
    
def plot_kdes(distribution,reference,labels, xlim = None, note="", print_stdevs=True):
    '''
    ...
    #xs = np.arange(glob.lbound[0],glob.rbound[0],0.001)
    if xlim is None:
        xs = np.linspace(glob.lbound[0],glob.rbound[0],500)
    else:
    ...
    axs.plot(xs,rys,color="black",linewidth=1,linestyle="dashed",
             label=labels[1])
    axs.set_title("Kernel density estimates %s" % note)
