...
def measure(t):
    ...
    if first_measure:
        ...
        print(" using 2*pi*f for the argument (f is not the angular frequency).",end="")
        print(" [function_evaluations.likelihoods.measure]")
        first_measure = False
    
    r = np.random.binomial(1, p=np.sum(np.cos(2*np.pi*real_parameters*t/2)**2/dim))
    return r
...
def simulate_1(particle, t):
    ...
    '''
    p=np.sum(np.cos(2*np.pi*particle*t/2)**2/glob.dim)
    return p 
...
def loggradient(data,particle,coef,autograd=False,ind=None):
    ...
    if autograd:
        ...
        for (t,outcome) in data:
            arg = 2*np.pi*particle*t/2
            L1 = np.sum(np.cos(arg)**2)/dim
            dL1 = -2*np.pi*t/dim*np.cos(arg)*np.sin(arg)
            if outcome==1:
                lg += coef*dL1/L1
            if outcome==0:
                L0,dL0 = 1-L1,-dL1
...
def sec_order_loggradient(data,particle,coef,autograd=False):
    ...
    if autograd:
        ...
        for (t,outcome) in data:
            ...
            L1 = np.sum(np.cos(arg)**2)/dim
            dL1 = -2*np.pi*t/dim*np.cos(arg)*np.sin(arg)
            ddL1 = -(2*np.pi*t)**2/(2*dim)*(-np.sin(arg)**2+np.cos(arg)**2)
            if outcome==1:
