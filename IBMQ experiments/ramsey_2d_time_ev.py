...
steps.
The evolution of the means of these quantities with time are plotted, with 
errorbars marking the standard deviations (medians are taken for both means and 
standard deviations).
"""
...
right_boundaries = np.array([5,1/3])
...
def bayes_update(data, distribution, moves=3, threshold=0.8):
    '''
...
def main():
    ...
    try:
          for i in range(datasets):
              ...
              filename = 'armonk_ramsey_data_ts[2.0,5[f=1.83_sched=75_nshots=2_' + str(i) + '.data'
              '''
              ...
              '''
              data,steps = get_data(filename=filename, every=1, upload=False, 
...
def plot_errorplot(xs, ys, dys, axs, ylabel="", units="", subject="",color="red"):
    axs.errorbar(xs, ys, dys, linestyle='None', marker='.', capsize=3,color=color)
    axs.set(xlabel='Time (s)', ylabel=(ylabel + f" ({units})"))
    #axs.axhline(y=f_real, color='r', linestyle='-')
    axs.set_title(subject + ' time evolution')
