# -*- coding: utf-8 -*-
"""
Quantum parameter learning implementation for estimating T2, the phase damping
...
T2_real = 0 
A,B = 0.5, 0.5 # Pre-determined parameters for the model.

left_boundaries = np.array([10])
right_boundaries = np.array([250])
...
def measure(t):
    ...
    '''
    r = np.random.binomial(1, p=(0.5*np.exp(-t/T2_real)+0.5))
    return r
...
def U_gradient(data,particle,autograd=False):
    ...
    if autograd:
        ...
        for (t,outcome) in data:
            L1 = 0.5*np.exp(-t/T2)+0.5
            DL1 = (t/T2**2)*0.5*np.exp(-t/T2)
            if outcome==1:
                DU += np.array([-DL1/L1])
            if outcome==0:
                L0 = 1-L1
...
def bayes_update(data, distribution, threshold=0.8, moves=1):
    '''
    ...
    if first_bayes_update is True:
        print("Bayes update: resampling threshold = %.1f, %d Markov moves" 
              % (threshold,moves))
...
def show_results(off_runs,off_dicts,T2s,parameters):
    '''
...
def get_data(upload=False, filename=None, steps=75, tmin=1, tmax=3.5,rev=False,
    ...
    if upload:
        ...
        with open(filename, 'rb') as filehandle: #3, 10
            ...
            # Update the model parameters if the data vector provides them.
            global A, B 
            if data[0][0]=='params':
                A, B = data[0][1]
                print("> Adjusted parameters to A=%.2f, B=%.2f from header." % 
                      (A,B))
                data = data[1:]
        ...
        if rand:
            ...
            if rev:
                ts = ts[::-1]
        print(("\n> Using randomly generated " if rand else 
              "\n> Using evenly spaced ordered ") + "times.")
        data = [(t,measure(t)) for t in ts]
        
    print(data)
    print("> Tmax = %.1f; steps = %d; T2max = %.1f." 
          % (tmax,steps,*right_boundaries))
    return data

def main():
    global T2_real, N_particles, right_boundaries, dim
    T2_max = right_boundaries[0]
    T2_real = 57
    ...
    for i in range(ndatasets):
        ...
        data = get_data(filename=filename, upload=False, steps=unique_ts, 
                        tmin=1, tmax=25, rev=True, rep=20, rand=False)
    ...
    parameters = [np.array([T2_real]) for i in range(runs)]
    T2s = []
    off_dicts = []
    try:
          for i in range(ndatasets):
              ...
              for j in range(runs_each):
                  ...
                  T2 = sequences[0][steps]
                  T2s.append(T2)
                  print("> Estimated T2=%.2f." % T2)
    ...
    if runs!=0:
        show_results(off_runs,off_dicts,T2s,parameters)
