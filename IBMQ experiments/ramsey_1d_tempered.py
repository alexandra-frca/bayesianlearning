...
import warnings
warnings.simplefilter("ignore") # error, ignore, default, always, once
np.seterr(all='warn')
dim=1
...
left_boundaries = np.array([0])
right_boundaries = np.array([10])
...
def measure(t, particle=np.array([f_real])):
    ...
    '''
    r = np.random.binomial(1, p=np.cos(2*np.pi*f_real*t/2)**2)
    return r
...
def U_gradient(data,particle,coef,autograd=False):
    ...
    if autograd:
        ...
        for (t,outcome) in data:
            ...
            if outcome==1:
                DU -= coef*np.array([DL1/L1])
            if outcome==0:
                L0 = 1-L1
                DL0 = -DL1
                DU -= coef*np.array([DL0/L0])
    return(DU)
...
def hamiltonian_MC_step(data, particle,coef, 
                        M=np.identity(2), L=20, eta=0.001, threshold=0):
    '''
...
def bayes_update(data, distribution, coef, previous_coef, threshold=0.5):
    '''
    ...
    if first_bayes_update:
      print("Bayes update:  resampling threshold = %.1f*N" % threshold)
    ...
    if (1/acc_squared_weight <= threshold*N_particles):
        ...
        # Perform a mutation step on each selected particle.
        distribution.clear()
        for key in selected_particles:
            particle = np.frombuffer(key,dtype='float64')
            mutated_particle = hamiltonian_MC_step(data,particle,coef,M=Cov_inv)
...
def main():
    ...
    try:
          for i in range(datasets):
              print(f"> Dataset {i}.")
              filename = 'echoed_ramsey_data[0.2,10[df=1.83_sched=75_nshots=1_'\
                  + str(i) + '.data'
              data,measurements = get_data(filename=filename, 
                                    upload=False, measurements=measurements,
                                     rep=1, tmin=0.2,tmax=10, rand=False)
