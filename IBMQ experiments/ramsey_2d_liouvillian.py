...
factor, using offline Bayesian inference. The time evolution of the qubit system
is computed according to a 2-parameter master equation using Liouvillian 
integration. Based on https://arxiv.org/pdf/1906.04478.pdf
...
import sys, copy, random, pickle, warnings, scipy.integrate, \
...
right_boundaries = np.array([10,0.5])
...
def simulate_1_exp(particle, t):
    ...
    the fixed form Hamiltonian and using a damped sinusoid model. 
...
def liouvillian_evolution(t,r,gamma,omega,E):
    '''
    Computes the time derivative of the density operator, by multiplying it by
    the Liouvillian superoperator. This is the RHS of the von-Neumann 
    differential equation, to be integrated for the time evolution.
    
    Parameters
    ----------
    t: float
        The instant at which the derivative is to be evaluated.
    r: float
        The density matrix
    ...
    '''
  r00,r01,r10,r11 = r
  dr00 = 1j*omega*r01 - 1j*omega*r10 + gamma*r11
  dr01 = 1j*omega*r00 + (+1j*E-gamma/2)*r01 - 1j*omega*r11
  dr10 = -1j*omega*r00 + (-1j*E-gamma/2)*r10 + 1j*omega*r11
  dr11 = -1j*omega*r01 + 1j*omega*r10 - gamma*r11
  dr = np.array([dr00, dr01, dr10, dr11], dtype=np.complex128)
  return dr

def simulate_1_rk45(particle, t):
    '''
    Provides an estimate for the likelihood  P(D=1|test_f,t) of an x-spin 
    measurement at time t yielding result |+>, given a set of parameters for 
    ...
    p: float
        The estimated probability of finding the particle at state |1>.
    '''
    f, gamma = particle*1e6
    omega = 2*np.pi*f/2
    E = 0
    #E = 3.3e-19
    r0 = np.array([0,0,0,1], dtype=np.complex128)
    t0, tmax = 0, t*1e-6
    RKint = scipy.integrate.RK45(lambda t,r: liouvillian_evolution(t,r,gamma,
                                                                   omega,E), 
                                t0, r0, tmax)

    while RKint.status=='running':
      RKint.step()
    if RKint.status=='failed':
      print("Runge Kutta integration failed.")
    else: # Status is 'finished'.
      r11 =  RKint.y[-1]

    return np.real(r11) 

def simulate_1(particle, t, integration_method='RK45', tols=(5e-3,5e-6)):
    '''
    Provides an estimate for the likelihood  P(D=1|test_f,t) of an x-spin 
    measurement at time t yielding result |+>, given a set of parameters for 
    the fixed form Hamiltonian and using numerical integration to simulate
    the time evolution given by a master equation. 
    ...
    integration_method: str, optional
        The numerical integration method to be used by Scipy (Default is 'RK45). 
    tols: (float,float), optional
        The absolute and relative tolerances for the integration result, by this
        order (Default is (5e-3,5e-6)).
    ...
    result = scipy.integrate.solve_ivp(lambda t,r: liouvillian_evolution(t,r,
                                                              gamma,omega,E),
                                      (0,t),r0,method=integration_method,
...
def metropolis_hastings_step(data, particle, S=np.identity(2),
    ...
    global dim
    Sigma = factor*S**0.25 # Numpy normal takes stdev, not variance.
...
def show_results(off_runs,off_dicts,fs,alphas,parameters):
    ...
    # Now median alpha.
    alpha_median = np.nanpercentile(alphas, 50, interpolation='nearest')
    alpha_median_index = alphas.index(alpha_median)
    median_alpha_dist = off_dicts[alpha_median_index]
    plot_distribution(median_alpha_dist,note=" (run with median alpha)")
    ...
    print("- f = %.4f" % median_f)
    print("- alpha = %.6f (T2* = %.4f)" % (median_alpha, 1/median_alpha))
...
def offline_estimation(distribution, data, plot=False):
    '''
...
def get_data(upload=False, filename=None, every=1, steps=75, rep=1,
    ...
    print("> t ∈[%.1f,%.1f[; steps = %d; fmax = %.1f; alphamax = %.2f (T2*min=%.2f)" 
           % (tmin,tmax,steps,*right_boundaries,1/right_boundaries[1]))
    return data,steps

def main():
    global f_real, alpha_real, N_particles, right_boundaries, dim
    f_max, alpha_max = right_boundaries
    f_real, alpha_real = 1.83, 1/6
    ...
    try:
          for i in range(datasets):
              print(f"> Dataset {i}.")
              filename = 'ramsey_data[0.2,5[df=1.83_sched=75_nshots=1_' + str(i) + '.data'
              '''
              ...
              data,steps = get_data(filename=filename, every=1, upload=True, 
                                 steps=75, rep=1,  tmin=0.2, tmax=4, rand=False)
