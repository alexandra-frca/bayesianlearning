        ...
        for particle in selected_particles:
            ...
            while (repeated == True):
                ...
                if (mutated_particle not in distribution):
                    repeated = False
            distribution[mutated_particle] = 1/N_particles
    return(distribution)
...
def offline_estimation(distribution, f_max, steps, increment=0.08):
    ...
    if first_offline_estimation is True:
        print("Offline estimation: increment=%.2f" % 
              (increment))
    ...
    #ts = [0.1*(9/8)**k for k in range(steps)]
...
def expected_utility(distribution, time):
    ...
    bayes_update([(time,0)],dist_0) 
    bayes_update([(time,1)],dist_1)
