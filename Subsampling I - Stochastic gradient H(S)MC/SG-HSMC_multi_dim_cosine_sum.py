...
subsampling a fraction of the observations for the resampling steps.
...
If subsampling, only stochastic gradient Hamiltonian Monte Carlo is used, and 
the Hamiltonian dynamics is adapted to improve convergence in the presence of 
noise. Additionally, the rejection steps are skipped. The integration error  
being uncorrected, the energy is no longer a constant, and the error scales with  
the stepsize, which must be kept small.
...
samples = None # Cardinality of the subsamples for computing the gradient.
...
def simulate_dynamics(data, coef, initial_momentum, initial_particle, M,L,eta,
    ...
    ----------
    data: [([float],int)]
        A vector of experimental results obtained and their respective controls, 
        each datum being of the form (time,outcome), where 'time' is the control 
        used for each experiment and 'outcome' is its result.
    coef: float
        The tempering coefficient to be used in computing the (annealed)
        likelihood.
...
def momentum_step(momentum,DU,B,C,eta,M_inv):
    '''
    Performs an update of the momentum (step resulting from the discretization 
    of Hamilton's equations' integration).
    ...
    momentum: [float]
        The starting momentum vector. 
    DU: [float]
        The gradient of the potential energy to be used for the update.
    B: `dim`x`dim` array
        The estimated noise model (a HMC tuning parameter).
    C: `dim`x`dim` array
        A custom friction term (a HMC tuning parameter).
    eta: float
        The integration stepsize (a HMC tuning parameter).
    M_inv: [[float]]
        The mass matrix/Euclidean metric to be used when simulating the 
        Hamiltonian dynamics (a HMC tuning parameter).
    ...
    momentum_components.append(np.linalg.multi_dot([-eta*C,M_inv,momentum]))
    momentum_components.append(np.random.multivariate_normal(np.zeros(dim),
                                                             2*(C-B)*eta))
    new_momentum = sum(momentum_components)   
    return new_momentum

first_simulate_dynamics_friction = True
def simulate_dynamics_friction(data, coef, initial_momentum, initial_particle, 
                      M,L,eta,V_est=100000,C=1500,
                      left_constraints=lbound, 
                      right_constraints=rbound):         
    '''
    Simulates Hamiltonian dynamics for a given particle, using leapfrog 
    integration.
    As compared to `simulate_dynamics`, it introduces friction (akin to the 
    Langevin equation's).
    
    Parameters
    ----------
    data: [([float],int)]
        A subsample of the obtained experimental results and their respective 
        controls, each datum being of the form (time,outcome), where 'time' is          
        the control used for each experiment and 'outcome' is its result.
    coef: float
        The tempering coefficient to be used in computing the (annealed)
    ...
    eta: float
        The integration stepsize to be used when simulating the Hamiltonian 
        dynamics (a HMC tuning parameter).
    V_est: float OR `dim`x`dim` array, optional
        The estimated noise model (a HMC tuning parameter) (Default is 10^4). If 
        one-dimensional it will be used to populate the diagonal of a 
        `dim`x`dim` array.
    C: float OR `dim`x`dim` array, optional
        A custom friction term (a HMC tuning parameter) (Default is 1500). (C-B) 
        should be positive semi-definite. If one-dimensional it will be used to 
        populate the diagonal of a `dim`x`dim` array.
    left_constraints: [float], optional
        The leftmost bounds to be enforced for the particle's motion (Default 
        is `lbound`).
    right_constraints: [float], optional
    ...
    B = 0.5*eta*V_est
    #B,C=0,0
    global first_simulate_dynamics_friction
    if first_simulate_dynamics_friction:
        if np.size(B)==1 and np.size(C)==1:
            print("Friction HMC: V_est=%f, B_est=%f, C=%f" % (V_est,B,C))
            first_simulate_dynamics_friction = False

    if np.size(B)==1:
        B = np.diag([B for d in range(dim)])
    if np.size(C)==1:
        C = np.diag([C for d in range(dim)])

    M_inv = np.linalg.inv(M)
    new_particle = initial_particle
    DU = measurements/samples*U_gradient(data,new_particle,coef)
    ...
    p = 1 # No MH acceptance-rejection step.
    return new_particle, p
        
first_hamiltonian_MC_step = True
def hamiltonian_MC_step(data, particle,coef,
                M=np.identity(dim), L=10, eta=0.01, 
                threshold=0.1,subsample=False):
    '''
    ...
    subsample: bool, optional 
        Whether to subsample (Default is False).
    ...
    if (threshold<1):
        ...
        if subsample:
            new_particle,p = simulate_dynamics_friction(data,coef,
                                             initial_momentum,particle,M,L,eta)
        else:
            new_particle, p = \
    ...
    #-likelihood).
    # This will never happen when subsampling because p=1 (skip the acceptance/
    #rejection step).
...
def bayes_update(data, distribution, coef, previous_coef, threshold,
                 subsample=False,signal_resampling=False):
    '''
    ...
    if (1/acc_squared_weight <= threshold):
        ...
        for key in selected_particles:
            ...
            while (repeated == True):
                particle = np.frombuffer(key,dtype='float64')
                indexes = [random.randrange(0,len(data)) \
                           for i in range(samples)]
                data_sample = [data[i] for i in indexes]
                selected_data = data_sample if subsample else data
                mutated_particle = hamiltonian_MC_step(selected_data,particle,
                                             coef,M=Cov_inv,subsample=subsample)
...
def SMCparameters(distribution, stdev=True, list=False):
    ...
    if stdev=="corrected": # Which also evaluates to True (non empty string).
        # Get the corrected sample standard deviation, which yields an unbiased 
        #variance.
...
def offline_estimation(distribution, data, tempering_coefficients, 
                       threshold=None, subsample=False, plot_all=False):
    '''
    ...
    for i,coef in enumerate(tempering_coefficients[1:],start=1):
        ...
        distribution, resampled = bayes_update(data, distribution, coef, 
                tempering_coefficients[i-1], threshold, 
                subsample=subsample,signal_resampling=True) 
...
def weighted_kernel_density_estimate(x, points, stdev):
    ...
    points: [([float],float)]
        ...
        vector.
    stdev: float 
        The standard deviation of the points, to be used to choose the bandwidth
        for the KDE.
    ...
    for point in points:
        p,w = point
        kde += w*gaussian(x,p,h,normalize=True)/(n*h)
    return(kde)
...
def plot_kdes(distribution,reference,labels):
    ...
    xs = np.arange(0.6,1,0.001)
    ys =  [weighted_kernel_density_estimate(x,ks,stdev) for x in xs]
    rys = [weighted_kernel_density_estimate(x,rks,rstdev) for x in xs]
...
def main():
    global real_parameters, N_particles, measurements, samples
    ...
    if test_subsampling:
        ...
        plot_distribution(final_dist_subs,real_parameters,
